...
use crate::{
    ...
    domains::{CouplingState, FactTable, ReborrowingState},
    PointwiseState,
};
...
pub struct ReborrowingAnalysis<'cpl, 'facts: 'cpl, 'mir: 'facts, 'tcx: 'mir> {
    tcx: TyCtxt<'tcx>,
    def_id: DefId,
...
impl<'cpl, 'facts: 'cpl, 'mir: 'facts, 'tcx: 'mir> ReborrowingAnalysis<'cpl, 'facts, 'mir, 'tcx> {
    pub fn new(
        tcx: TyCtxt<'tcx>,
        def_id: DefId,
        fact_table: &'facts FactTable<'tcx>,
...
impl<'cpl, 'facts: 'cpl, 'mir: 'facts, 'tcx: 'mir> FixpointEngine<'mir, 'tcx>
    for ReborrowingAnalysis<'cpl, 'facts, 'mir, 'tcx>
{
    type State = ReborrowingState<'cpl, 'facts, 'mir, 'tcx>;
    ...
    fn new_bottom(&self) -> Self::State {
        ...
        Self::State::new_bottom(self.body_with_facts, self.fact_table, self.coupling)
    }

    fn new_initial(&self) -> Self::State {
        Self::State::new_empty(self.body_with_facts, self.fact_table, self.coupling)
    }

...
pub(super) fn fix_patch<'tcx>(body: &mir::Body<'tcx>, mut patch: MirPatch<'tcx>,) -> MirPatch<'tcx> {
    assert_eq!(patch.patch_map.len(), body.basic_blocks().len() + patch.new_blocks.len());
    ...
    while new_block_offset > 0 {
        ...
        if !reachable_blocks.contains(&bb) {
            ...
            eprintln!("patch.patch_map: {:?} bb: {:?}", patch.patch_map, bb);
            eprintln!("patch new_blocks: {} new_block_offset: {}", patch.new_blocks.len(), new_block_offset);
            ...
            for new_bb_index in new_block_offset..(patch.new_blocks.len()) {
                ...
                // Shift all patch_map.
                for terminator in &mut patch.patch_map {
                    if let Some(terminator) = terminator {
